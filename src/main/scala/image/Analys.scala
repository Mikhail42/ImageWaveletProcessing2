...
object Analys {
  ...
  private def twoMaxHistogramtInds(gi: AInt) = {
    ...
    val sum1 = (5 to indAbsMax).map(gi).sum
    val sum2 = (indAbsMax until 250).map(gi).sum
    val indLocMax =
      if (sum1 > sum2) {
        val locMax = (5 to 3 * indAbsMax / 5).map(gi).max
        5 + (5 to 3 * indAbsMax / 5).map(gi).indexOf(locMax)
      } else {
        val locMax = (indAbsMax + (250 - indAbsMax) * 2 / 5 until 250).map(gi).max
        indAbsMax + (indAbsMax + (250 - indAbsMax) * 2 / 5 until 250).map(gi).indexOf(locMax)
      }
  ...
  private def minHistogramInd(gi: AInt) = {
    ...
    println(ind1, ind2)
    val mn = (ind1 to ind2).map(gi).min
    ind1 + (ind1 to ind2).map(gi).indexOf(mn)
  }
  ...
  def direction(mat: MInt, R: Int = 5, steptheta: Int = 5, extr: String = "ARGMIN"): MInt = {
    ...
    val allDYs = (0 until 180 by steptheta).map(dys)
    val allDXs = (0 until 180 by steptheta).map(dxs)
    ...
    def argMinDisp(i: Int, j: Int): Int = {
      val disps = (0 until 180 by steptheta).map {
        getDisp(i, j, _)
      }
  ...
  def mediate(directly: MInt, matToUpdate: MInt): Unit = {
    val m = directly.length;
    val n = directly(0).length
  ...
  def getMediateLine(imgMat: MInt, x: Int, y: Int, theta: Int): (Int, Int) = {
    ...
    val cosA = coss(theta + 90);
    val sinA = sins(theta + 90)
